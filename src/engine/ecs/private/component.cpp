...
void IComponent::resize_component_memory(size_t new_size, ActorVariant* variant, size_t component_type_index) const
{
    ...
    else if (new_size < variant->per_component_data_size[component_type_index] / 2)
    {
        const size_t element_size = type_size();

        LOG_WARNING("resize down from %d to %d", variant->per_component_data_size[component_type_index], variant->per_component_data_size[component_type_index] / 2);
    ...
    {
        delete[] variant->component_data[component_type_index];
        variant->component_data[component_type_index] = nullptr;
        variant->per_component_data_size[component_type_index] = 0;
    }
}
} // namespace ecs