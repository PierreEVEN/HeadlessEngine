...
{
    if (parent)
        rel_rotation = inverse(parent->get_world_transform()) * mat4_cast(in_rotation); //@TODO : ensure it works
    else
...
{
    if (parent)
        rel_scale = glm::dvec3(1, 1, 1) / parent->get_world_scale() * in_scale; //@TODO : ensure it works
    else
        rel_scale = in_scale;
