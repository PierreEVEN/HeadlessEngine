...
Surface* Surface::create_surface(application::window::Window* container)
{
    ...
    if (!RenderPass::find("resolve_pass"))
        RenderPass::declare(RenderPass::Config{
    ...
    surface->main_render_pass = RenderPassInstance::create(container->width(), container->height(), RenderPass::find("resolve_pass"), std::vector{surface->get_surface_render_texture()});
    return surface;
#else
    return nullptr;
#endif
