...
{
VkImageUsageFlags vk_usage(const TextureParameter& texture_parameters)
{
...
ImageResource_VK::ImageResource_VK(const std::string& name, const CI_Texture& create_infos) : image_layout(VK_IMAGE_LAYOUT_UNDEFINED), image_infos(create_infos)
{
    ...
    VK_CHECK(vmaCreateImage(get_vma_allocator(), &image_create_infos, &vma_allocation, &image, &memory, nullptr), "failed to create image");
    ...
    debug_set_object_name(stringutils::format("image:%s", name.c_str()), image);
}
...
SamplerResource_VK::SamplerResource_VK(const std::string& name, [[maybe_unused]] const CreateInfos& create_infos)
{
    ...
    VK_CHECK(vkCreateSampler(get_device(), &sampler_create_infos, get_allocator(), &sampler), "failed to create sampler");
    debug_set_object_name(name, sampler);
...
SamplerResource_VK::~SamplerResource_VK()
{
...
ImageViewResource_VK::ImageViewResource_VK(const std::string& name, const CI_TextureView& create_infos) : used_image(create_infos.used_image)
{
